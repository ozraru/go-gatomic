...
func ExampleValue_config() {
	var config gatomic.Value[map[string]string] // holds current server configuration
...
func ExampleValue_readMostly() {
	...
	insert := func(key, val string) {
		...
		defer mu.Unlock()
		m1 := m.Load()  // load current value of the data structure
